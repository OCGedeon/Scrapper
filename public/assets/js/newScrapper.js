    ...
    $('.savedArticles').on("click", () => { 
      ...
      fetch("/api/savedArticles", {method: "GET"}).then(response => response.json()).then((response) => {
        ...
        response.map(article => {
          ...
          sessionStorage.setItem(article["_id"], JSON.stringify(article)) // Store Article Data
            ...
            fetch("/api/populateNote", { // Send savedArticle to the Server
              ...
              if (data.length >= 1) {
                data.map((comment) => {
                  ...
                  if (comment === null) {
                    let notesDiv = "<div class='col s12 m7'><div class='card horizontal'><div class='card-image'><img src='https://i.pinimg.com/originals/29/a0/d9/29a0d9f43e2cad8bf4d360f812449abb.jpg'></div><div class='card-stacked center'><div class='card-content valign-wrapper'><p>No Notes.</p></div></div></div></div>";
                    $(".boxComments").prepend(notesDiv);
                  } else {
                    let notesDiv = "<div class='col s12 m7' id='" + comment["_id"] + "'><div class='card horizontal'><div class='card-image'><img src='https://i.pinimg.com/originals/29/a0/d9/29a0d9f43e2cad8bf4d360f812449abb.jpg'></div><div class='card-stacked center'><div class='card-content valign-wrapper'><p>" + comment.body + "</p></div><div class='card-action deleteComment' data-id=" + comment["_id"] + "><a href='#'>Delete</a></div></div></div></div>";
                    $(".boxComments").prepend(notesDiv);
                  }
          ...
          $(".deleteArticle").on("click", function(event) { // Event Listenr For Saved Article Delete Button
            ...
            fetch("/api/deleteArticle", { // Send savedArticle to the Server
              ...
              console.log(response)
              $("#modalMessage").modal('open');
              $("#modalMessage .modal-content ").html('<h4> Deleted:' + sessionArticle["_id"] + "</h4>");
